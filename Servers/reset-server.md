...
Sends the reset operation to a list of servers and adds operation to queue. (See <a href="/knowledge-base/servers/descriptions-of-servergroup-power-commands/">Description of Server Group Power Commands</a> for details on how the reset operation is used.) Calls to this operation must include a token acquired from the authentication endpoint. See the <a href="/api-docs/v2#authentication-login">Login API</a> for information on acquiring this token.

### When to Use It

Use this API operation when you want to reset a single server or group of servers. It should be used in conjunction with the <a href="/api-docs/v2#queue-get-status">Get Status</a> operation to check the result of the reset command.
...
### Structure
    ...
    https://api.tier3.com/v2/operations/{accountAlias}/servers/reset

### Example

    https://api.tier3.com/v2/operations/ALIAS/servers/reset
...
<table>
  ...
  <tbody>
    <tr>
      <td>serverIds</td>
      <td>array</td>
      <td>List of server IDs to perform reset operation on.</td>
      <td>Yes</td>
    </tr>
  </tbody>
</table>
