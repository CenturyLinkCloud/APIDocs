...
<p>A: While SSL ensures that the message cannot be read in transit, it doesn't protect you from rogue callers who target your public endpoint. Each Webhook notification includes a signature hash of the message payload. The "Tier3-RsaSha1" header is encrypted
  with our private key and can be decrypted with our public key. After decrypting the signature, customers can compare the payloads to make sure that the message wasn't tampered with.</p>
<p>&nbsp;</p>
